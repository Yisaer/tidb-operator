...
func GenerateRSAKeyPair(namespace string) ([]byte, []byte, error) {
	ca := &x509.Certificate{
	...
	hosts := []string{"admission-controller-svc." + namespace + ".svc"}
	for _, h := range hosts {
		server.DNSNames = append(server.DNSNames, h)
	}
	privSer, _ := rsa.GenerateKey(rand.Reader, 2048)
	return createCertificateFile(server, privSer, ca, privCa)
}
